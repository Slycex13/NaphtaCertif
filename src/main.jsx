...
import { getTheme, defaultStore, themes } from './data'
import { backendEnabled, loadStore, resetLocalStore, saveAttempt, saveQuestion, seedRemoteQuestions, supabase } from './storage'
import './styles.css'

const iconMap = { activity: Activity, wind: Wind, flame: Flame, 'git-branch': GitBranch, thermometer: Thermometer, shield: Shield }
...
function App() {
  ...
  const [sessionConfig, setSessionConfig] = useState({
    name: '', team: 'Équipe 1', role: 'Opérateur extérieur', mode: 'Évaluation', count: 8,
  ...
  const startSession = (event) => {
    ...
    const filtered = sessionConfig.themeIds.length ? pool.filter((question) => sessionConfig.themeIds.includes(question.themeId)) : pool
    if (!filtered.length) {
  ...
  const finishSession = async (finalAnswers) => {
    ...
    const attempt = {
      id: makeId('ATT'), employeeName: quiz.config.name.trim(), team: quiz.config.team, role: quiz.config.role,
...
function EmployeeHome({ config, setConfig, store, onStart, onAdmin }) {
  const publishedCount = store.questions.filter((question) => question.status === 'published').length
  ...
  return <div className="employee-page page-enter">
    ...
    <section className="start-layout">
      <div className="section-heading"><div><span className="eyebrow compact"><span className="eyebrow-line" />Lancer une session</span><h2>Quel terrain voulez-vous réactiver ?</h2></div><div className="question-availability"><BookOpen size={16} /><span>{publishedCount || store.questions.length} questions disponibles</span></div></div>
      <form className="session-card" onSubmit={onStart}>
        <div className="form-main">
          ...
          <Field label="Fonction"><select value={config.role} onChange={(event) => update('role', event.target.value)}>{['Opérateur extérieur', 'Consoliste / Chef de poste', 'Électricien HT / Dispatching', 'Chef de quart', 'Formateur'].map((item) => <option key={item}>{item}</option>)}</select></Field>
      ...
      <div className="bottom-notes"><div><span className="note-index">A1</span><span><strong>Référentiel source</strong><small>EXP-U-15-002 à 007 · supports Utilities</small></span></div><div><span className="note-index orange">!</span><span><strong>Certitude obligatoire</strong><small>Chaque réponse inclut votre niveau de confiance.</small></span></div><button className="text-button" type="button" onClick={onAdmin}>Accéder au suivi admin <ArrowRight size={15} /></button></div>
    </section>
  </div>
}
...
function AdminResults({ store, onNotify }) {
  ...
  const attempts = store.attempts.filter((attempt) => attempt.employeeName.toLowerCase().includes(query.toLowerCase()) && (teamFilter === 'Toutes les équipes' || attempt.team === teamFilter))
  const exportCsv = () => {
    const lines = [['ID', 'Date', 'Collaborateur', 'Équipe', 'Fonction', 'Mode', 'Score', 'Surconfiances', 'Critiques manquées'], ...attempts.map((attempt) => [attempt.id, attempt.completedAt, attempt.employeeName, attempt.team, attempt.role, attempt.mode, `${attempt.score}%`, attempt.overconfidence, attempt.criticalFailures])]
...
function QuestionEditor({ question, onClose, onSave }) {
  const [draft, setDraft] = useState(question || { id: 'Q-NEW', themeId: 'gaz', status: 'draft', critical: false, prompt: '', choices: ['', '', '', ''], correctIndex: 0, rationale: '', source: 'EXP-U-15-002' })
  ...
  const submit = (event) => { event.preventDefault(); if (!draft.prompt.trim() || draft.choices.some((choice) => !choice.trim()) || !draft.rationale.trim()) return; onSave({ ...draft, id: draft.id === 'Q-NEW' ? makeId('Q') : draft.id }) }
  return <div className="modal-backdrop"><div className="editor-modal"><div className="editor-header"><div><span className="eyebrow compact"><span className="eyebrow-line" />{question ? 'Modifier' : 'Nouveau'} référentiel</span><h2>{question ? question.id : 'Nouvelle question'}</h2></div><button className="modal-close" onClick={onClose} aria-label="Fermer"><X size={18} /></button></div><form onSubmit={submit}><div className="editor-grid"><Field label="Thème"><select value={draft.themeId} onChange={(event) => update('themeId', event.target.value)}>{themes.map((theme) => <option key={theme.id} value={theme.id}>{theme.name}</option>)}</select></Field><Field label="Référence source"><input value={draft.source} onChange={(event) => update('source', event.target.value)} placeholder="EXP-U-15-002" /></Field></div><label className="check-line"><input type="checkbox" checked={draft.critical} onChange={(event) => update('critical', event.target.checked)} /><span><strong>Situation critique</strong><small>Prioriser cette question en révision ciblée.</small></span></label><Field label="Question"><textarea rows="3" required value={draft.prompt} onChange={(event) => update('prompt', event.target.value)} placeholder="Formulez une situation observable sur le terrain…" /></Field><div className="field-label">Réponses <small>Choisissez la bonne réponse</small></div><div className="choice-editor">{draft.choices.map((choice, index) => <div className={`choice-input ${draft.correctIndex === index ? 'correct' : ''}`} key={index}><button type="button" onClick={() => update('correctIndex', index)} aria-label={`Définir la réponse ${index + 1} comme correcte`}>{draft.correctIndex === index ? <Check size={15} /> : String.fromCharCode(65 + index)}</button><input required value={choice} onChange={(event) => updateChoice(index, event.target.value)} placeholder={`Réponse ${String.fromCharCode(65 + index)}`} /></div>)}</div><Field label="Rappel de consigne"><textarea rows="3" required value={draft.rationale} onChange={(event) => update('rationale', event.target.value)} placeholder="Expliquez le raisonnement attendu…" /></Field><div className="editor-footer"><div className="publish-select"><span className="field-label">Statut de publication</span><select value={draft.status} onChange={(event) => update('status', event.target.value)}><option value="draft">Brouillon</option><option value="published">Publiée</option></select></div><div className="editor-actions"><button type="button" className="modal-cancel" onClick={onClose}>Annuler</button><button className="primary-button" type="submit">Enregistrer <Check size={16} /></button></div></div></form></div></div>
}
